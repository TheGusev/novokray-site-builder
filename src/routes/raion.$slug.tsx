  ...
  head: ({ loaderData, params }) => {
    ...
    return {
      meta: [
        ...
        { property: "og:url", content: `/raion/${params.slug}` },
          ...
          "@graph": [
            {
              ...
              areaServed: { "@type": "AdministrativeArea", name: d.full, containedInPlace: { "@type": "City", name: SITE.city } },
              address: { "@type": "PostalAddress", addressCountry: "RU", addressRegion: SITE.region, addressLocality: SITE.city },
            ...
            },
            {
              ...
              itemListElement: [
                ...
                { "@type": "ListItem", position: 2, name: "Районы Новосибирска", item: `${SITE.domain}/karta-sayta` },
                { "@type": "ListItem", position: 3, name: d.full, item: `${SITE.domain}/raion/${params.slug}` },
              ],
            },
          ],
        }),
...
function DistrictPage() {
  ...
  const faq = [
    { q: `Вы работаете ${d.prepositional}?`, a: `Да. Бригада Дез-Федерация выезжает ${d.prepositional} ежедневно с 07:00 до 23:00. Время в пути — до 60 минут.` },
    { q: `Сколько стоит обработка ${d.prepositional}?`, a: `Выезд бесплатный, оплачивается только обработка. Цена фиксируется до приезда — от 1 500 ₽ за квартиру.` },
    { q: `Какие услуги доступны ${d.prepositional}?`, a: `Все 13 направлений: клопы, тараканы, грызуны, плесень, озонирование, сушка после потопов, обработка участков, фумигация.` },
    { q: `Даёте ли гарантию?`, a: `Да, гарантия по договору — до 12 месяцев на уничтожение вредителей и до 24 месяцев на обработку от плесени.` },
    { q: `Работаете с ТСЖ и УК ${d.prepositional}?`, a: `Да. Заключаем договоры с УК и ТСЖ, ведём журнал по СанПиН, обрабатываем подъезды, подвалы и придомовые территории.` },
  ];
  ...
  return (
    <>
        ...
        <div className="container-x relative grid gap-8 py-12 md:py-16 lg:grid-cols-[1.25fr,1fr]">
          <div>
            ...
            <p className="speakable mt-5 max-w-2xl text-[15px] leading-relaxed text-white/90 md:text-lg">
              {d.description} Выезжаем за 60 минут, работаем по договору, цена фиксируется до приезда. На каждую обработку — гарантия и бесплатная повторная выездка при возврате проблемы.
            </p>
          ...
          <div id="zayavka">
            <LeadForm variant="hero" title={`Заявка ${d.prepositional}`} subtitle="Перезвоним за 10 минут, зафиксируем цену." />
          </div>
        </div>
      </section>
      ...
      <section className="container-x py-14">
        <h2 className="font-display text-2xl font-bold md:text-3xl">Что обрабатываем {d.prepositional}</h2>
        <p className="mt-2 max-w-3xl text-muted-foreground">Все 13 направлений санитарной службы доступны жителям {d.genitive}. Самые востребованные — ниже.</p>
      ...
      <section className="bg-surface py-14">
        <div className="container-x">
          <h2 className="font-display text-2xl font-bold md:text-3xl">Где работаем {d.prepositional}</h2>
          <div className="mt-5 flex flex-wrap gap-2">
            {d.landmarks.map((l) => (
              <span key={l} className="inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 text-sm">
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {[
              { icon: Truck, t: "Выезд за 60 минут", d: "Бригада с оборудованием — день в день. Бесплатная диагностика на месте." },
              { icon: ShieldCheck, t: "Гарантия по договору", d: "До 24 месяцев. При возврате проблемы — приезжаем повторно бесплатно." },
              { icon: Clock, t: "Ежедневно 07:00–23:00", d: "Принимаем заявки в выходные и праздники. Аварийная сушка — круглосуточно." },
            ].map((b, i) => (
      ...
      <section className="container-x py-14">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {DISTRICTS.filter((x) => x.slug !== d.slug).map((x) => (
            <Link key={x.slug} to="/raion/$slug" params={{ slug: x.slug }} className="rounded-full border border-border bg-card px-4 py-2 text-sm font-semibold hover:border-primary hover:text-primary">
              {x.name}
            </Link>
          ))}
        </div>
