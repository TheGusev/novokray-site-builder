                ...
                hasOfferCatalog: {
                  ...
                  itemListElement: s.prices.map((p, i) => ({
                    ...
                    warranty: {
                      ...
                      durationOfWarranty: { "@type": "QuantitativeValue", value: warranty },
                    },
                  })),
                },
              },
              ...
              },
              {
                ...
                itemListElement: [
                  ...
                  },
                  {
                    ...
                    position: 3,
                    name: s.title,
                    item: `${SITE.domain}/services/${params.slug}`,
                  },
                ],
              },
              {
    ...
    <>
      ...
      <section className="relative overflow-hidden bg-hero text-primary-foreground">
        {hero && (
          <>
            <img
              src={hero}
              alt={imgMeta?.heroAlt ?? s.h1}
              title={imgMeta?.heroTitle ?? s.title}
        ...
        <div
          ...
          style={{
            backgroundImage:
              "radial-gradient(circle at 80% 30%, oklch(0.7 0.20 38 / 0.3), transparent 60%)",
          }}
        />
        <div className="container-x relative grid gap-8 py-10 md:py-14 lg:grid-cols-[1.25fr_1fr] lg:py-20">
          <div>
            ...
            <AnimatedHeading
              as="h1"
              text={s.h1}
              highlight="Новосибирске"
              className="mt-5 font-display text-[30px] font-extrabold leading-tight text-balance md:text-5xl"
            />
            <Reveal delay={250}>
            ...
            <Reveal delay={350} className="mt-7 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
              <a
                href={SITE.phoneHref}
                className="cta-shine inline-flex items-center justify-center gap-2 rounded-xl bg-cta-gradient px-5 py-4 font-bold text-accent-foreground shadow-cta hover:scale-[1.02] transition"
              >
          ...
          <div id="zayavka" className="lg:pl-6">
            <Reveal variant="scale" delay={120}>
              <LeadForm
                ...
                subtitle="Перезвоним в течение 10 минут и зафиксируем цену."
        ...
        <section className="container-x pt-12">
          <div className="grid gap-4 md:grid-cols-3">
            <div className="relative overflow-hidden rounded-2xl md:col-span-2 aspect-[16/10]">
              <img
                src={hero}
                alt={imgMeta?.cardAlt ?? `${s.title} — фото работ`}
                title={imgMeta?.cardTitle ?? `${s.title} — пример работы Дез-Федерация`}
                loading="lazy"
            ...
            <div className="grid gap-4">
              <div className="relative overflow-hidden rounded-2xl aspect-[4/3]">
                <img
                  src={COMMON.equipment}
                  alt={`Оборудование для услуги «${s.title}» — ULV-генератор и СИЗ`}
                  title={COMMON_IMAGE_META.equipment.title}
                  loading="lazy"
                  className="h-full w-full object-cover"
                />
              </div>
              <div className="relative overflow-hidden rounded-2xl aspect-[4/3]">
                <img
                  src={COMMON.documents}
                  alt={`Договор и акт по услуге «${s.title}» — закрывающие документы`}
                  title={COMMON_IMAGE_META.documents.title}
                  loading="lazy"
                  className="h-full w-full object-cover"
                />
              </div>
        ...
        <AnimatedHeading
          ...
          text={
            s.slug === "unichtozhenie-klopov"
              ? "Признаки клопов в квартире"
              : `Когда заказывать: ${s.title.toLowerCase()}`
          }
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2">
          {s.problems.map((p, i) => (
            <Reveal
              key={i}
              delay={i * 70}
              className="flex gap-3 rounded-xl border border-border bg-card p-5 shadow-card transition hover:-translate-y-0.5 hover:shadow-elegant"
            >
        ...
        {s.slug === "unichtozhenie-klopov" && (
          <Reveal className="mt-8 overflow-hidden rounded-2xl border border-border bg-card shadow-card">
            ...
            <div className="px-5 py-4 text-sm text-muted-foreground">
              {KLOPY_PHOTO_META.naMatrase.caption}
            </div>
          </Reveal>
        )}
        {s.slug === "unichtozhenie-klopov" && (
          <Reveal className="mt-8 max-w-3xl text-[15px] leading-relaxed text-muted-foreground">
            <p>
              Клопы редко приходят одни: если в доме заодно завелись кухонные насекомые, вместе с
              обработкой спальных мест имеет смысл заказать{" "}
              <Link
                to="/services/$slug"
                params={{ slug: "unichtozhenie-tarakanov" }}
              ...
              <Link
                to="/services/$slug"
                params={{ slug: "sushka-posle-zatopleniya" }}
                className="font-semibold text-primary underline-offset-4 hover:underline"
              >
              ...
              <Link
                to="/services/$slug"
                params={{ slug: "obrabotka-ot-pleseni" }}
                className="font-semibold text-primary underline-offset-4 hover:underline"
              >
                плесень и грибок
              </Link>
      ...
      <section className="bg-surface py-16">
        <div className="container-x">
          <AnimatedHeading
            ...
            text={
              s.slug === "unichtozhenie-klopov"
                ? "Как проходит обработка от клопов"
          ...
          <div className="mt-8 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
            {s.steps.map((st, i) => (
              ...
              >
                <div className="font-display text-3xl font-extrabold text-primary/30">
          ...
          {s.slug === "unichtozhenie-klopov" && (
            <div className="mt-10 grid items-stretch gap-6 lg:grid-cols-2">
              <Reveal className="overflow-hidden rounded-2xl border border-border bg-card shadow-card">
                ...
                />
                <div className="px-5 py-4 text-sm text-muted-foreground">
                  {KLOPY_PHOTO_META.razborKrovati.caption}
                </div>
              </Reveal>
              <div className="grid gap-5 sm:grid-cols-1">
                ...
                >
                  <div className="font-display text-lg font-bold text-primary">
                    Препараты без запаха, эффект с первых минут
                  </div>
                  <p className="mt-2 text-sm text-muted-foreground">
                    Работаем составами без резкого запаха: гибель клопов начинается уже во время
                    обработки, а вернуться в квартиру можно в тот же день — после 3–4 часов
                    проветривания. Мебель, техника и текстиль не портятся, следов на поверхностях не
                    остаётся.
                  </p>
                </Reveal>
                <Reveal
                ...
                >
                  <div className="font-display text-lg font-bold text-primary">
                    Точечная обработка спальных мест
                  </div>
                  <p className="mt-2 text-sm text-muted-foreground">
                    Кровать и диван разбираем: проливаем ламели, короб, изнанку основания, стыки
                    каркаса и ножки. Матрас обрабатываем по швам и кантам, отдельно проходим
                    изголовье, плинтусы и дверные коробки — именно там держится гнездо, а не на
      ...
      <section className="container-x py-16">
        ...
        <AnimatedHeading
          ...
          text={
            s.slug === "unichtozhenie-klopov"
              ? "Технологии и препараты против клопов"
      ...
      <section className="bg-surface py-16">
        <div className="container-x">
          ...
          <p className="mt-3 max-w-2xl text-muted-foreground">
            Стоимость фиксируется до выезда. Без скрытых платежей и доплат за препараты.
          </p>
              ...
              <tbody className="divide-y divide-border">
                {s.prices.map((p, i) => (
                  <tr key={i} className="text-sm">
                    ...
                    <td className="whitespace-nowrap px-3 py-3 text-right font-display font-bold text-foreground md:px-5 md:py-4">
                      {p.price.replace(/\s/g, "\u00a0")}
                    </td>
                  </tr>
                ))}
        ...
        <div className="grid gap-8 rounded-3xl border border-border bg-card p-8 shadow-card md:grid-cols-2 md:p-12">
          <div>
            ...
            <p className="mt-3 text-muted-foreground">
              Выдаём договор и акт выполненных работ. Если проблема вернётся в течение срока
              гарантии — приезжаем повторно бесплатно. Работаем по 152-ФЗ, лицензия
              Роспотребнадзора.
            </p>
            <Link
              to="/garantii"
              className="mt-5 inline-flex items-center gap-1 text-sm font-semibold text-primary hover:gap-2"
            >
              Подробнее о гарантиях <ArrowRight className="h-4 w-4" />
            </Link>
          </div>
          <LeadForm
            title="Записаться на обработку"
            subtitle="Заявка ни к чему не обязывает. Сначала озвучим цену."
            defaultService={s.title}
          />
        </div>
      </section>
      ...
      <FAQ
        ...
        title={
          s.slug === "unichtozhenie-klopov"
            ? "Частые вопросы про обработку от клопов"
      ...
      {related.length > 0 && (
        <section className="container-x pb-16">
          ...
          <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {related.map((r) => (
              <ServiceCard key={r.slug} service={{ ...r, icon: getServiceIcon(r.slug) }} />
            ))}
          </div>
        </section>
      )}
