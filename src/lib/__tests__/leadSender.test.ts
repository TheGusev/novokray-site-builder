...
describe("normalizePhone", () => {
  it("нормализует 8XXXXXXXXXX", () => {
    expect(normalizePhone("89069989888")).toBe("+79069989888");
  });
  it("нормализует форматированный номер", () => {
    expect(normalizePhone("+7 (906) 998-98-88")).toBe("+79069989888");
  });
...
describe("buildLeadBody", () => {
  it("подставляет honeypot и нормализованный телефон", () => {
    const body = buildLeadBody({ type: "Заявка на обработку", phone: "8 906 998 98 88", pest: "Клопы" });
    expect(body.phone).toBe("+79069989888");
