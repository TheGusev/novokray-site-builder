...
export function typo(input: string, options: TypoOptions = {}): string {
  ...
  // диапазон чисел с пробелами: «5 - 7» → «5–7» (до правила длинного тире)
  s = s.replace(/(\d)\s+[-\u2013]\s+(?=\d)/g, `$1${NDASH}`);
  ...
  if (!options.plain) {
    ...
    s = s.replace(/(?<![А-Яа-яЁёA-Za-z])(т|г|ул|д|стр|кв|руб|тыс|млн)\.\s+/g, `$1.${NBSP}`);
